...
def home():
    if request.method == "POST":
        ...
        status = Cursor(
            api.search, q=request.form["recipe"], tweet_mode="extended", lang="en"
        ).items(10)
    ...
    # Get title, image, id of food
    response = requests.get(
        f"https://api.spoonacular.com/recipes/complexSearch?apiKey={spoonKey}&type=dessert&sort=random&fillIngredients=true&number=1&titleMatch="
        + dessertList[index[0]]
    )
    rep = json.loads(response.text)
    title = rep["results"][0]["title"]
    ...
    food = dessertList[index[0]] + " -filter:retweets"

    # Create Cursor object and search for tweets
    status = Cursor(api.search, q=food, tweet_mode="extended", lang="en").items(10)
...
if __name__ == "__main__":
    app.run(
        debug=False, port=int(os.getenv("PORT", 8080)), host=os.getenv("IP", "0.0.0.0")
    )
